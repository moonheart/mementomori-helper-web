...
function GuildCard({ guild, rank, onClick, highlightColor, valueLabel, value }: GuildCardProps) {
    ...
    const getLeaderAvatarUrl = (): string => {
        ...
        // 使用 CharacterIconManager 获取头像 URL
        const { isSpecialIcon, characterId } = CharacterIconManager.decodeIconId(iconId);
        if (isSpecialIcon) {
            return CharacterIconManager.getSpecialIconUrl(characterId);
        }
...
function PlayerCard({ player, rank, currentUserId, onClick, highlightColor, valueLabel, value }: PlayerCardProps) {
    ...
    // 如果是特殊头像，加载 SpecialIconItemMB 获取实际角色ID
    ...
    // 计算头像URL
    // 特殊头像: 使用 SpecialIconItemMB.characterId，URL后缀固定为001
    // 普通头像: 直接使用解码后的 characterId
    ...
    return (
        <div
            className={`flex flex-col p-4 rounded-lg border cursor-pointer transition-all hover:bg-accent ${
        ...
        async function fetchRankingData() {
            try {
                ...
                if (playerResponse) {
                    ...
                    if (playerResponse.userSyncData) {
                        setUserData(playerResponse.userSyncData);
                    }
                }
                ...
                if (towerResponse) {
                    ...
                    if (!userData && towerResponse.userSyncData) {
                        setUserData(towerResponse.userSyncData);
                    }
                }
            } catch (error) {
            ...
            <Tabs value={mainTab} onValueChange={setMainTab} className="space-y-6">
                ...
                <TabsContent value="player" className="space-y-4">
                    <Tabs value={playerSubTab} onValueChange={setPlayerSubTab} className="space-y-4">
                        ...
                        {/* 战力排行榜 */}
                        <TabsContent value="player-power" className="space-y-4">
                            <Card>
                                <CardHeader>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                        {playerData?.battlePowerList.map((player, index) => (
                                            <PlayerCard
                                                ...
                                                valueLabel="战力"
                                                value={player.battlePower.toLocaleString()}
                                            />
                                        ))}
                                    </div>
                                </CardContent>
                        ...
                        {/* 等级排行榜 */}
                        <TabsContent value="player-level" className="space-y-4">
                            <Card>
                                <CardHeader>
                                    <CardTitle className="flex items-center gap-2">
                        ...
                        {/* 主线冒险排行榜 */}
                        <TabsContent value="player-quest" className="space-y-4">
                            <Card>
                                <CardHeader>
                                    <CardTitle className="flex items-center gap-2">
                        ...
                        {/* 无穷之塔排行榜 */}
                        <TabsContent value="player-tower-infinite" className="space-y-4">
                            <Card>
                                <CardHeader>
                                    <CardTitle className="flex items-center gap-2">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                        {playerData?.towerBattleList.map((player, index) => (
                                            <PlayerCard
                                                ...
                                                valueLabel="层数"
                                                value={`第${player.latestTowerBattleQuestId}层`}
                                            />
                                        ))}
                                    </div>
                                </CardContent>
                        ...
                        {/* 元素塔排行榜 */}
                        <TabsContent value="player-tower" className="space-y-4">
                            <Card>
                                <CardHeader>
                                    <CardTitle className="flex items-center gap-2">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                        {guildData?.levelList.map((guild, index) => (
                                            <GuildCard
                                                ...
                                                highlightColor="green"
                                                valueLabel="总战力"
                                                value={guild.battlePower.toLocaleString()}
                                            />
                                        ))}
                                    </div>
                                </CardContent>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                                        {guildData?.stockList.map((guild, index) => (
                                            <GuildCard
                                                ...
                                                highlightColor="blue"
                                                valueLabel="今日积分"
                                                value={guild.guildStock.toLocaleString()}
                                            />
                                        ))}
                                    </div>
                                </CardContent>
            ...
            <Dialog open={guildDialogOpen} onOpenChange={setGuildDialogOpen}>
                <DialogContent>
                    ...
                    {selectedGuild && (
                        <div className="space-y-4">
                            <div className="flex items-center gap-4">
                                <div className={`text-4xl font-bold ${getRankColor(selectedGuild.rank)}`}>
                                    #{selectedGuild.rank}
                                </div>
                                <div>
